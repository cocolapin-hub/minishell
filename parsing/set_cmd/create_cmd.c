...
/*   Updated: 2025/11/05 11:09:44 by ochkaoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     new->heredoc_fd = -1;  // Initialize this!
//     new->next = NULL;
//     return (new);
// }
...
void create_cmd(t_token **tmp, t_token **new, t_token **start, t_token **end)
{
    ...
    // KEY FIX: We need to copy the REDIRECTION token, not the target WORD token!
    *new = malloc(sizeof(t_token));
    if (!*new)
        return;

    (*new)->type = (*tmp)->type;  // Use the redirection type (REDIR_IN, etc.)
    (*new)->value = ft_strdup(target->value);  // But copy the target's value
    if (!(*new)->value)
    {
        free(*new);
        *new = NULL;
